...
drop view if exists dbo.israel;
go
create view dbo.israel as 
...
--order by o.empid, orderyear;
go
select * from dbo.israel order by empid, orderyear;
go
...
-- Write a query against Sales.VEmpOrders (renamed israel)
-- that returns the running qty for each employee and year
-- Tables involved: TSQLV4 database, Sales.VEmpOrders view

--try to write a scalar valued query against israil for sum(qty)
select sum(qty) from dbo.israel;

select * from dbo.israel order by empid, orderyear;
---------------------------------------------------
select sum(i1.qty) as runqty from dbo.israel i1 where i1.empid = 1
and i1.orderyear <= 2015
---------------------------------------------------
select i.empid, i.orderyear, i.qty, 
(select sum(i1.qty) as runqty from dbo.israel i1 where i.empid = i1.empid
and i1.orderyear <= i.orderyear) as runqty
from israel i
order by i.empid, i.orderyear;
